...
bool CCameraDS::OpenCamera(int nCamID,int *w,int *h, bool bDisplayProperties)
{
...
Img* CCameraDS::QueryFrame()
{
	...
	//}
	if(!m_pFrame){
		m_pFrame = (Img*)malloc(sizeof(Img));
		m_pFrame->data = (unsigned char*)malloc(size);
		memset(m_pFrame->data,0x0,size);
		m_pFrame->size = size;
		m_nBufferSize = size;
